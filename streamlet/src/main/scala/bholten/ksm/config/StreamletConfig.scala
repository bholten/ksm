...
object StreamletConfig {
  ...
  def loadDefault[F[_]: Sync]: F[StreamletConfig] =
    Sync[F].blocking(ConfigFactory.load).map(c => StreamletConfig(c))
...
final case class StreamletConfig(config: Config) {
  ...
  val startUpHandler: bholten.ksm.streams.api.StartupHandler = {
    val className = config.getString("streamlet.startup.handler")
    this.getClass.getClassLoader
      .loadClass(className)
      .getDeclaredConstructor()
      .newInstance()
      .asInstanceOf[bholten.ksm.streams.api.StartupHandler]
  }
